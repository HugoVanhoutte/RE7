...
<form action="../../public/index.php/recipe?action=validateEdit&id=<?= $recipe->getId() ?>" method="post">
    ...
    <div>
        ...
        <input type="number" max="65535" name="preparation_time_minutes" id="preparation_time_minutes" value="<?= $recipe->getPreparationTimeMinutes() ?>">
    </div>

    <div>
        <label for="cooking_time_minutes">Temps de cuisson (en minutes)</label>
        <input type="number" max="65535" name="cooking_time_minutes" id="cooking_time_minutes" value="<?= $recipe->getCookingTimeMinutes() ?>">
    </div>

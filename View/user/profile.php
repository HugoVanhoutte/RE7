...
<div class="container">
    ...
    if (isset($_SESSION['user_id']) && (new UserManager())->isEditable($user->getId())) //Checks if user is authenticated (prevent error message) and check if he is admin or user
    {
        ?>
        <div>
            <a href="../index.php/user?action=edit&id=<?= $user->getId() ?>" title="&Eacute;diter mon profil" class="btn btn-outline-primary">
                <i class="fa-solid fa-pen"></i></a>
            <a href="../index.php/user?action=delete&id=<?= $user->getId() ?>" title="Supprimer mon compte" class="btn btn-outline-danger">
            ...
            foreach ($recipes as $recipe) {
                ...
                <div class="col-12 col-md-5 col-lg-4 col-xl-3">
                    <div class="card bg-light rounded shadow m-2 text-center border-0">
                        ...
                        <p class="text-muted fst-italic">le <?= $recipeManager->getTimeFR($recipe->getCreationDateTime()) ?></p>
                        <?php
        ...
        <div class="row align-items-center justify-content-center">
            ...
            <div class="col-12 col-md-5 col-lg-4 col-xl-3">
                <div class="card bg-light rounded shadow m-2 text-center border-0">
                    ...
                    <p class="text-muted fst-italic">le <?= $commentManager->getTimeFR($comment->getCreationDateTime()) ?> à propos de <a
                                href="../../public/index.php/recipe?action=view&id=<?= $recipe->getId() ?>"><?= $recipe->getTitle() ?></a>
                    </p>
                    <?php
                    if (isset($_SESSION['user_id']) && (new UserManager())->isEditable($user->getId())) {
                    ?>
                    <a href="../../public/index.php/comment?action=edit&id=<?= $comment->getId() ?>"
                       title="&Eacute;diter" class="btn btn-outline-primary"><i class="fa-solid fa-pen"></i></a>

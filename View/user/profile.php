...
<ul>
    ...
    if (empty($recipes)){
    ?>
    <h3 class="empty_list"><?= $user->getUsername() ?> n'a pas encore publié de recettes</h3>
    <?php
    }
    ...
    foreach ($recipes as $recipe) {
        /* @var Recipe $recipe */
        ?>
        <li><a href="../../public/index.php/recipe?action=view&id=<?= $recipe->getId() ?>"><?= $recipe->getTitle() ?></a>
            <?php
...
<ul>
    ...
    if (empty($comments)) {
        ?>
            <h3 class="empty_list"><?= $user->getUsername() ?> n'a pas encore publié de commentaires</h3>
            <?php
    }
    ...
    foreach ($comments as $comment) {
        ...
        <div>
            ...
            <p>le <?= $commentManager->getTimeFR($comment->getCreationDateTime()) ?> à propos de <?= $recipe->getTitle() ?></p>
        </div>
    <?php
    }
    ?>
